...
public function Factura2($data){
  ...
  $oi=$oi+$n1+$n1;
  printer_draw_text($handle, iconv("UTF-8", "ISO-8859-1", $data["nombre"]), 425, $oi);
  printer_draw_text($handle, date("d") . " " . Fechas::MesEscrito(date("m")) ." de " . date("Y"), 4125, $oi-2);
  
  $oi=$oi+$n1;
  printer_draw_text($handle, iconv("UTF-8", "ISO-8859-1",  $data["direccion"]), 475, $oi+3);
  
  printer_draw_text($handle, $data["documento"], 525, $oi+$n1);
  
  
  $oi=1500; // salto de linea
